...
func setupTestFileStorageDatabase(t *testing.T) (*core.DatabasePool, func()) {
	t.Helper()
	...
	postgresContainer, err := tcpostgres.Run(ctx,
		"postgres:18-alpine",
		tcpostgres.WithDatabase("layr_filestorage_test"),
	...
	if err := db.RunMigrations(ctx, Migrations); err != nil {
		db.Close()
		_ = postgresContainer.Terminate(context.Background())
		t.Fatalf("failed to run file storage migrations: %v", err)
	}
...
func createTestServiceAccountWithS3Credentials(
	...
	encryptedSecretKey, err := cryptoKeyManager.EncryptField([]byte(secretAccessKey))
	if err != nil {
	...
	`
	_, err = db.Exec(ctx, insertCredentialSQL, parsedID, accessKeyID, encryptedSecretKey)
	if err != nil {
