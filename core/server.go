...
func NewServer(db *DatabasePool, cryptoKeyManager *CryptoKeyManager) *HTTPServer {
	...
	GetRoute[TopologyResponse](router, "/api/v1/topology", server.handleTopologyRequest,
		...
		RouteDescription("Returns dynamic cluster topology, enabled service flags, project metadata, and publishable client key for SDK initialization."),
...
// PublishableKeyMiddleware validates X-Layr-Publishable-Key (or Service Account fallback) on public /api/v1/* routes.
func (server *HTTPServer) PublishableKeyMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(responseWriter http.ResponseWriter, request *http.Request) {
		...
		if server.cryptoKeyManager != nil {
			publishableKey := request.Header.Get("X-Layr-Publishable-Key")
