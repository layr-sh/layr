...
func TestCoreCryptoEncryptionKeyRotationIntegration(t *testing.T) {
	...
	testPayloads := []struct {
		...
		{name: "standard string", payload: []byte("smtp-password-to-rotate-12345")},
		{name: "empty payload", payload: []byte("")},
		{name: "large payload", payload: bytes.Repeat([]byte("large-payload-block-for-rotation-"), 250)},
	}

	t.Run("payload rotation cycles", func(t *testing.T) {
		for _, testCase := range testPayloads {
			t.Run(testCase.name, func(t *testing.T) {
				secretPayload := testCase.payload
	...
	t.Run("tampering during key rotation pipeline", func(t *testing.T) {
