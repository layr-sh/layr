...
func TestThreatRiskDatabaseEvaluationIntegration(t *testing.T) {
	...
	firstRiskAssessment, firstErr := EvaluateSignInRisk(ctx, db, nil, userID, "1.2.3.4", "Mozilla/5.0 (Macintosh)", 30)
	if firstErr != nil {
		t.Fatalf("unexpected error on first evaluation: %v", firstErr)
	}
	...
	knownRiskAssessment, knownErr := EvaluateSignInRisk(ctx, db, nil, userID, "1.2.3.4", "Mozilla/5.0 (Macintosh)", 30)
	if knownErr != nil {
		t.Fatalf("unexpected error on known evaluation: %v", knownErr)
	}
	...
	newDeviceRiskAssessment, newDeviceErr := EvaluateSignInRisk(ctx, db, nil, userID, "1.2.3.4", "Mozilla/5.0 (Windows)", 30)
	if newDeviceErr != nil {
		t.Fatalf("unexpected error on new device evaluation: %v", newDeviceErr)
	}
	...
	newIPRiskAssessment, newIPErr := EvaluateSignInRisk(ctx, db, nil, userID, "9.9.9.9", "Mozilla/5.0 (Macintosh)", 30)
	if newIPErr != nil {
		t.Fatalf("unexpected error on new IP evaluation: %v", newIPErr)
	}
	...
	highRiskAssessment, highRiskErr := EvaluateSignInRisk(ctx, db, nil, userID, "9.9.9.9", "Mozilla/5.0 (iPhone)", 30)
	if highRiskErr != nil {
		t.Fatalf("unexpected error on high risk evaluation: %v", highRiskErr)
	}
...
func TestThreatRiskBrokenPoolIntegration(t *testing.T) {
	...
	riskAssessment, err := EvaluateSignInRisk(ctx, brokenDB, nil, "0191eb70-0000-7000-8000-000000000001", "1.2.3.4", "Mozilla/5.0", 30)
	if err != nil {
		t.Fatalf("expected fail-safe nil error with broken pool, got: %v", err)
	}
