...
func TestAuthHandlerOAuthLifecycleIntegration(t *testing.T) {
	...
	handler.HandleOAuthAuthorize(oauthAuthResponseResponseRecorder, oauthAuthRequest)
	...
	handler.HandleOAuthCallback(callbackResponseResponseRecorder, callbackRequest)
	...
	handler.HandleOAuthCallback(secondCallbackResponseResponseRecorder, secondCallbackRequest)
	...
	handler.HandleOAuthUserInfo(userInfoResponseResponseRecorder, userInfoRequest)
	...
	nonExistentToken, nonExistentTokenErr := handler.signer.GenerateAccessToken(jwt.Claims{
	...
	handler.HandleOAuthUserInfo(nonExistentUserInfoResponseRecorder, nonExistentUserInfoRequest)
	...
	anonAccessToken, err := handler.signer.GenerateAccessToken(jwt.Claims{
		Subject:     anonUserID,
		Role:        "authenticated",
		IsAnonymous: true,
	}, 900)
	...
	handler.HandleOAuthAuthorize(anonAuthorizeResponseRecorder, anonAuthorizeRequest)
	...
	handler.HandleOAuthCallback(conversionResponseResponseRecorder, conversionRequest)
	...
	handler.HandleOAuthCallback(conflictResponseResponseRecorder, conflictRequest)
	...
	handler.HandleOAuthCallback(emailConflictResponseResponseRecorder, emailConflictRequest)
	...
	handler.HandleOAuthCallback(oidcLinkResponseResponseRecorder, oidcLinkRequest)
	...
	brokenHandler.HandleOAuthCallback(brokenResponseRecorder, brokenRequest)
	...
	brokenHandler.HandleOAuthUserInfo(brokenUserInfoResponseRecorder, brokenUserInfoRequest)
	...
	if claims := handler.resolveCustomClaims(context.Background(), testUserClaimsID); claims != nil {
		t.Fatalf("expected nil claims before function creation, got: %+v", claims)
	}
	if claims := handler.resolveCustomClaims(context.Background(), ""); claims != nil {
	...
	_, _ = db.Exec(context.Background(), `
		...
		$$;
	`)
	claims := handler.resolveCustomClaims(context.Background(), testUserClaimsID)
	...
	authedUserID, authErr := handler.authenticateUser(dbSessionRequest)
	...
	if anonUserRecord, resolveAnonymousCallerErr := handler.resolveAnonymousCaller(regularAuthRequest); anonUserRecord != nil || !errors.Is(resolveAnonymousCallerErr, ErrAnonymousSessionNotFound) {
	...
	if anonUserRecord, resolveAnonymousCallerErr := brokenHandler.resolveAnonymousCaller(regularAuthRequest); anonUserRecord != nil || resolveAnonymousCallerErr == nil || errors.Is(resolveAnonymousCallerErr, ErrAnonymousSessionNotFound) {
	...
	failAnonToken, _ := handler.signer.GenerateAccessToken(jwt.Claims{
