...
func TestAuthMFAFlowIntegration(t *testing.T) {
	...
	handler.handleMFAVerify(wrongVerifyResponseRecorder, wrongVerifyRequest)
	...
	currentTOTPCode, generateCodeErr := handler.GetTOTPManager().GenerateCode(mfaSetupResponse.Secret, time.Now())
	...
	handler.handleMFAVerify(validVerifyResponseRecorder, validVerifyRequest)
	...
	handler.handleMFAVerify(lockedVerifyResponseRecorder, lockedVerifyRequest)
	...
	handler.handleMFAVerify(corruptedVerifyResponseRecorder, corruptedVerifyRequest)
	...
	handler.handleMFAVerify(noSecretVerifyResponseRecorder, noSecretVerifyRequest)
	...
	handler.handleMFAVerify(nonExistentVerifyResponseRecorder, nonExistentVerifyRequest)
	...
	handler.handleMFASetup(nonExistentSetupResponseRecorder, nonExistentSetupRequest)
	...
	handler.handleMFASetup(phoneSetupResponseRecorder, phoneSetupRequest)
	...
	phoneTOTPCode, _ := handler.GetTOTPManager().GenerateCode(phoneMFASetupResponse.Secret, time.Now())
	...
	handler.handleMFAVerify(phoneVerifyResponseRecorder, phoneVerifyRequest)
