...
func TestAuthHandlerFullLifecycleIntegration(t *testing.T) {
	...
	handler.handleSignUp(signupResponseRecorder, signupRequest)
	...
	handler.handleSignIn(loginResponseRecorder, loginRequest)
	...
	handler.handleTokenRefresh(refreshResponseRecorder, refreshRequest)
	...
	handler.handleOTPVerify(missingOTPResponseRecorder, missingOTPRequest)
	...
	handler.handlePasswordResetRequest(resetResponseRecorder, resetRequest)
	...
	handler.handlePasswordResetConfirm(wrongConfirmResponseRecorder, wrongConfirmRequest)
	...
	handler.handlePasskeySignUp(passkeyOptionsResponseRecorder, passkeyOptionsRequest)
	...
	handler.handlePasskeySignUpVerify(passkeyVerifyResponseRecorder, passkeyVerifyRequest)
	...
	handler.handlePasskeySignIn(passkeySignInOptionsResponseRecorder, passkeySignInOptionsRequest)
	...
	handler.handlePasskeySignInVerify(passkeySignInVerifyResponseRecorder, passkeySignInVerifyRequest)
	...
	handler.handleMFASetup(mfaSetupResponseRecorder, mfaSetupRequest)
	...
	currentTOTPCode, err := handler.GetTOTPManager().GenerateCode(mfaSetupResponse.Secret, time.Now())
	if err == nil && currentTOTPCode != "" {
		...
		handler.handleMFAVerify(mfaVerifyResponseRecorder, mfaVerifyRequest)
	...
	handler.HandleOAuthCallback(oauthCallbackResponseRecorder, oauthCallbackRequest)
	...
	handler.handleUserExport(exportResponseRecorder, exportRequest)
	...
	nonExistentToken, tokenErr := handler.signer.GenerateAccessToken(jwt.Claims{
	...
	handler.handleUserExport(nonExistentExportResponseRecorder, nonExistentExportRequest)
...
func TestAuthAnonymousSignInAndInPlaceConversionIntegration(t *testing.T) {
	...
	handler.handleAnonymousSignIn(anonymousResponseRecorder, anonymousRequest)
	...
	anonymousClaims, claimErr := handler.signer.VerifyAccessToken(anonymousSessionResponse.AccessToken)
	if claimErr != nil {
	...
	handler.handleSignUp(signupConversionResponseRecorder, signupConversionRequest)
	...
	convertedClaims, tokenErr := handler.signer.VerifyAccessToken(signupConversionSessionResponse.AccessToken)
	if tokenErr != nil {
	...
	handler.handleSignUp(brandNewResponseRecorder, brandNewRequest)
	...
	handler.handleAnonymousSignIn(secondAnonymousResponseRecorder, secondAnonymousRequest)
	...
	handler.handleAnonymousSignIn(thirdAnonymousResponseRecorder, thirdAnonymousRequest)
	...
	handler.handleUpdateUserEmail(patchEmailResponseRecorder, patchEmailRequest)
	...
	handler.handleAnonymousSignIn(fourthAnonymousResponseRecorder, fourthAnonymousRequest)
	...
	handler.handleUpdateUserPhone(patchPhoneResponseRecorder, patchPhoneRequest)
...
func TestAuthHandlerCredentialsAndSessionFlowsIntegration(t *testing.T) {
	...
	handler.handleSignIn(wrongPassResponseRecorder, wrongPassRequest)
	...
	handler.handleSignIn(missingUserResponseRecorder, missingUserRequest)
	...
	handler.handleSignIn(validPhoneResponseRecorder, validPhoneRequest)
	...
	for i := 0; i < 2; i++ {
		...
		handler.handleSignIn(rateAttemptResponseRecorder, rateAttemptRequest)
	}
	...
	handler.handleSignIn(rateBlockedResponseRecorder, rateBlockedRequest)
	...
	handler.handleSignIn(lockedSignInResponseRecorder, lockedSignInRequest)
	...
	handler.handleSignUp(conflictSignupResponseRecorder, conflictSignupRequest)
	...
	handler.handleAnonymousSignIn(anonResponseRecorder, anonRequest)
	...
	handler.handleSignUp(conflictEmailResponseRecorder, conflictEmailRequest)
	...
	handler.handleSignUp(conflictPhoneResponseRecorder, conflictPhoneRequest)
	...
	handler.handleSignUp(blockConvertResponseRecorder, blockConvertRequest)
	...
	handler.handleAnonymousSignIn(canceledAnonResponseRecorder, canceledAnonRequest)
	...
	handler.handleSignUp(canceledSignupResponseRecorder, canceledSignupRequest)
	...
	handler.handleTokenRefresh(revokedRefreshResponseRecorder, revokedRefreshRequest)
	...
	handler.handleTokenRefresh(expiredRefreshResponseRecorder, expiredRefreshRequest)
	...
	handler.handleTokenRefresh(deletedUserRefreshResponseRecorder, deletedUserRefreshRequest)
	...
	handler.handleSignIn(validDBSignInResponseRecorder, validDBSignInRequest)
	...
	handler.handleTokenRefresh(validDBRefreshResponseRecorder, validDBRefreshRequest)
